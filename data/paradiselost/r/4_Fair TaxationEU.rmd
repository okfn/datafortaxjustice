...
```{r}
countries <- read.csv("data/List of EU Countries.csv", stringsAsFactors = FALSE, strip.white = TRUE)
...
taxrates$LOCATION <- gsub("Slovakia", "Slovak Republic", taxrates$LOCATION)
...
indicators <- read.csv("data/indicators EU.csv", stringsAsFactors = FALSE, sep = ";", strip.white = TRUE)
...
colnames(countries) <- c("jurisdiction", "passes fair taxation", "is a sink of a conduit (with sink centrality value)", "witholding tax rate on dividens", "has a zero percent rate?", "has a preferential tax regime?", "Corporate tax rate (2017)", "Corporate tax rate ranking", "Has transfer pricing rules?", "has thin cap rules?", "has CFC rules?")

write.csv(countries, file = "outcomes/Criterium Fair Taxation-EU.csv", row.names = FALSE)
...
write.csv(countries, file = "outcomes/Criterium Fair Taxation-EU (with X'es).csv", row.names = FALSE)
write.xlsx(countries, file = "final blacklist/final blacklist-EU.xlsx", sheetName = "Fair Taxation", row.names = FALSE, col.names = TRUE, append = TRUE, showNA = FALSE)
```
